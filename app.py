...
if uploaded_file is not None:
    ...
    #dill.dump(data,open('scrap_data.txt','wb'))
    data=preprocssor.preprocess(data)
    #dill.dump(df,open('scrap_df.txt','wb'))

    user=data['user'].unique().tolist()
    ...
    if st.sidebar.button('Show Analysis'):
        ...
        with col1:
            ...
            st.title(helper.number_msg(selected_user,data))

        with col2:
            st.header('Total Words') 
            st.title(helper.number_words(selected_user,data)) 

        with col3:  
            st.header('Media Shared')
            st.title(helper.media(selected_user,data)) 

        with col4:
            st.header('Links Shared')
            st.title(helper.links(selected_user,data))  

        if selected_user=='Overall':
            st.title("Most Busy User")
            x,new_df=helper.most_busy_user(data)
            fig,ax=plt.subplots()
        ...
        most_common_words=helper.most_words(selected_user,data) 
        fig,ax=plt.subplots()
        ...
        timeline=helper.moth_wise_msg(selected_user,data)
        fig,ax=plt.subplots()
        ...
        with col1:
            ...
            busy_day=helper.most_busy_day(selected_user,data)
            fig,ax=plt.subplots()
        ...
        with col2:
            ...
            busy_month=helper.month_activity(selected_user,data)
            fig,ax=plt.subplots()
